...
// Metadata precedence sources, in ABS's canonical default order. Labels mirror
// the ABS scanner tab.
...
export function LibraryEditModal({
  ...
  const { data: providersData } = useQuery({
    queryKey: adminKeys.searchProviders(target.serverId),
  ...
  const addNewFolder = async () => {
    ...
    const result = await checkFolderExists(target, p)
          ...
          <div className="cfg-card">
            ...
            <div className="cfg-line" style={{ gap: 12, alignItems: 'flex-end' }}>
              ...
              <input
                ...
                onChange={(e) =>
                  patchS({ markAsFinishedValue: e.target.value.replace(/[^0-9]/g, '') })
                }
              />
            </div>
          </div>
