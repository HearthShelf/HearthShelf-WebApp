...
export function RequestsPage() {
  ...
  // Route gate: the request layer only exists when an admin has connected RMAB on
  // the active server. A stale /requests link redirects home when it's off.
  const { data: config, isLoading: configLoading } = useRmabConfig()

  const { data, isLoading, isError, refetch } = useQuery({
    queryKey: ['rmab', 'requests', target?.serverId, tab],
    queryFn: () => listRequests(target!, tab),
    enabled: Boolean(target) && config?.enabled === true,
