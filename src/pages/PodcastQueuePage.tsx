...
import { getDownloadQueue, clearDownloadQueue } from '@/api/absPodcasts'
...
export function PodcastQueuePage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['podcast-download-queue', target?.serverId, activeId],
    queryFn: () => getDownloadQueue(target!, activeId as string),
  ...
  const clear = async (podcastItemId: string) => {
    if (!target) return
    await clearDownloadQueue(target, podcastItemId)
    qc.invalidateQueries({ queryKey: ['podcast-download-queue', target.serverId, activeId] })
  }
