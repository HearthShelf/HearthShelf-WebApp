...
export function ReaderPage({ itemId: itemIdProp, inline, onClose }: ReaderPageProps = {}) {
  ...
  const title = detail?.title ?? 'Reading'
  ...
  const ui = useMediaUI()
  const sessionId = player.now?.itemId ?? null
  const isPlaying = player.playing
  const audioCurrent = player.positionSec
  const audioDuration = player.now?.totalDurationSec ?? 0
  const audioOn = !!itemId && sessionId === itemId
  const hasAudio = (detail?.audioFiles.length ?? 0) > 0

  // Move the reading position to roughly where the audio is. epub.js maps a
  // percentage through the book to a CFI; the audio's elapsed fraction is the
  // best cross-medium anchor we have (no per-word sync).
  ...
  useEffect(() => {
    if (!itemId || !target || !viewerRef.current) return
    ...
    // cross-origin request with the per-server ABS token (Bearer header) just
    // like any other absClient fetch.
    const origin = target.serverUrl.replace(/\/$/, '')
    ...
    // Recreate when the book or the flow-affecting layout changes. The token is
    // read fresh (getAbsToken) so auth refreshes don't churn the rendition.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [itemId, target?.serverId, target?.serverUrl, prefs.layout])
