...
export function NarratorsPage() {
  ...
  const enabled = Boolean(target) && activeId !== null

  const { data, isLoading, isError, refetch } = useQuery({
    queryKey: ['narrators', target?.serverId, activeId],
    queryFn: () => getNarrators(target!, activeId as string),
    enabled,
    staleTime: 5 * 60 * 1000,
  })

  const { data: itemsData } = useQuery({
    queryKey: ['narrators-all-items', target?.serverId, activeId],
    queryFn: () => getAllLibraryItems(target!, activeId as string),
    enabled,
    staleTime: 5 * 60 * 1000,
  })
  ...
  const invalidate = () =>
    qc.invalidateQueries({ queryKey: ['narrators', target?.serverId, activeId] })
  ...
  const doSave = async (patch: { name: string }) => {
    if (!editing || !target || !activeId) return
    setBusy(true)
    try {
      await renameNarrator(target, activeId, editing.name, patch.name)
      await invalidate()
      show('Narrator saved')
      setEditing(null)
    } catch {
