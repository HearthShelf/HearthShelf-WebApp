...
export function ConfigMeta() {
  ...
  const { data: tags, isLoading: tagsLoading } = useQuery({
    queryKey: adminContentKeys.tags(target?.serverId ?? ''),
    queryFn: () => getAllTags(target!),
    enabled: Boolean(target),
    staleTime: 60 * 1000,
  })
  const { data: genres, isLoading: genresLoading } = useQuery({
    queryKey: adminContentKeys.genres(target?.serverId ?? ''),
    queryFn: () => getAllGenres(target!),
    enabled: Boolean(target),
    staleTime: 60 * 1000,
  })
  ...
  const invalidate = () => {
    ...
    const key =
      kind === 'tags'
        ? adminContentKeys.tags(target.serverId)
        : adminContentKeys.genres(target.serverId)
    qc.invalidateQueries({ queryKey: key })
  }

  const doRename = async () => {
    if (!target || !renaming || !renameVal.trim()) return
    const v = renameVal.trim()
    if (kind === 'tags') await renameTag(target, renaming, v)
    else await renameGenre(target, renaming, v)
    setRenaming(null)
    invalidate()
  }
  const doDelete = async (value: string) => {
    if (!target) return
    if (kind === 'tags') await deleteTag(target, value)
    else await deleteGenre(target, value)
    invalidate()
  }
