...
export function ConfigNotifications() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: adminContentKeys.notifications(target?.serverId ?? ''),
...
function NotificationsForm({ settings }: { settings: ABSNotificationSettings }) {
  ...
  const save = async () => {
    ...
    setSaving(true)
    try {
      await updateNotifications(target, {
        appriseApiUrl: apprise || null,
        maxFailedAttempts: Number(maxFailed) || 5,
        notificationDelay: Number(delay) || 1000,
      })
      qc.invalidateQueries({ queryKey: adminContentKeys.notifications(target.serverId) })
      setSaved(true)
  ...
  const toggleRule = async (id: string, enabled: boolean) => {
    if (!target) return
    await updateNotificationRule(target, id, { enabled })
    qc.invalidateQueries({ queryKey: adminContentKeys.notifications(target.serverId) })
  }
