...
import { getLatestEpisodes } from '@/api/absPodcasts'
...
export function PodcastLatestPage() {
  ...
  const { data, isLoading, isError, refetch } = useQuery({
    queryKey: ['recent-episodes', target?.serverId, activeId],
    queryFn: () => getLatestEpisodes(target!, activeId as string, 50),
        ...
        <div className="ep-list">
          {episodes.map((ep) => {
            const card: EpisodeCardData = {
              ...
              podTitle: ep.podcastTitle,
              played: progressById.get(ep.libraryItemId)?.progress ?? 0,
            }
            return (
              <EpisodeCard
                key={ep.id}
                ...
                onPlay={(e) =>
                  void playEpisode({
                    itemId: e.podcastItemId,
                    episodeId: e.id,
                    episodeTitle: e.title,
                    podcastTitle: e.podTitle,
                  })
                }
                onToast={show}
              />
