...
export function QuestGiverPage() {
  ...
  // Route gate: QuestGiver is default-enabled and shares the Discover config.
  // A stale /questgiver link redirects home only when the backend explicitly
  // disables it (featureEnabled === false).
  const featureEnabled = useQuestGiverEnabled()

  const { data: itemsData, isLoading } = useQuery({
    queryKey: ['questgiver', 'all-items', target?.serverId, activeId],
    queryFn: () => getAllLibraryItemsFull(target!, activeId as string),
    enabled: Boolean(target) && activeId !== null && featureEnabled,
  })
  ...
  useEffect(() => {
    let cancelled = false
    fetchServerRuns(target).then((serverRuns) => {
  ...
  const fetchExternalHits = async (terms: string[]): Promise<QgExternalHit[]> => {
    if (!target) return []
    const results = await Promise.allSettled(terms.map((t) => searchCatalog(target, t)))
  ...
  if (!featureEnabled) return <Navigate to="/" replace />
