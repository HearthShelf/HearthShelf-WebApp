...
export function UploadPage() {
  ...
  const { data: libraries = [], isLoading: librariesLoading } = useQuery({
    ...
    queryFn: () => getLibraries(target as NonNullable<typeof target>),
  ...
  const folders = useMemo(() => selectedLibrary?.folders ?? [], [selectedLibrary])
  ...
  const submit = async () => {
    ...
    if (ok) {
      ...
      qc.invalidateQueries({ queryKey: ['abs-libraries', target.serverId] })
      qc.invalidateQueries({ queryKey: ['abs-items', target.serverId] })
    }
  }
