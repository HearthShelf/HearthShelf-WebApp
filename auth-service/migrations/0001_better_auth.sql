-- Better Auth schema for HearthShelf accounts.
-- GENERATED by better-auth's own migration compiler (better-auth 1.7.2) against
-- the exact plugin set in src/auth.ts: passkey, twoFactor, emailOTP, magicLink,
-- username, bearer. Do not hand-edit - regenerate if the plugin list changes.
--
-- user.id holds each migrated account's ORIGINAL Clerk user id (see
-- scripts/seed-legacy.mjs). It is the grant `sub` and the per-user primary key
-- on every self-hosted box, so it must never be regenerated.
